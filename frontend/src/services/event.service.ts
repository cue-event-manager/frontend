...
import type { CancelEventRequestDto } from "@/domain/event/CancelEventRequestDto";
...
export const cancelEvent = async (id: number, request?: CancelEventRequestDto): Promise<Event> => {
    const { data } = await axiosInstance.delete<Event>(
