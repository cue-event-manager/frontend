...
export function SpaceResourcesTableFilter() {
    const { t } = useTranslation();
    const { query, updateQuery, resetQuery } =
        usePaginatedFilters<SpaceResourcePaginationRequestDto>();
    ...
    const {
        ...
        resetFilters,
        setFilters,
    } = useFilterForm<SpaceResourcePaginationRequestDto>({
        name: query.name ?? "",
    });
    ...
    return (
        <DataTableFilterBar onSearch={handleSearch} onClear={handleClear}>
            <TextField
                label={t("admin.spaceResources.filters.name")}
                variant="outlined"
                size="small"
                value={filters.name ?? ""}
