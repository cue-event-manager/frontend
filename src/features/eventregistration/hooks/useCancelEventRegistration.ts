...
import { cancelRegistrationToEvent } from "@/services/eventRegistration.service";
...
export function useCancelEventRegistration() {
    ...
    return useMutation({
        mutationFn: (payload: CancelEventRegistrationRequestDto) => cancelRegistrationToEvent(payload),
        onSuccess: async () => {
            toast.success(t("events.messages.registrationCancelled"));
            await Promise.all([
